...
if(isset($_POST['modelid']))
{
  ...
  if($modelid > 0)
  {
    ...
    $edit_product_types_stmt = $pdo->query("SELECT * FROM `products` where ID='$modelid'");
    if($edit_product_types_stmt->rowCount() > 0)
    {
      $edit_product_types_stmt_r = $edit_product_types_stmt->fetch();
      $product_name = $edit_product_types_stmt_r['product_name'];
      $product_type_id = $edit_product_types_stmt_r['product_type_id'];
      $product_image = $edit_product_types_stmt_r['product_image'];
    }
  }else{
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <input type="text" id="product_name" name="product_name" placeholder="Enter Product" class="form-control" value="<?=$product_name?>">
      </div>
...
if(isset($_POST['deleted_id']))
{
  ...
  if($pdo->query("DELETE FROM `products` WHERE ID='$delete_id'") == true)
  {
    echo "1";
  }else{
    echo "0";
...
if(isset($_POST['page']))
{
  ...
  $product_types_stmt = $pdo->query("SELECT * FROM `products`");
      ...
      while($product_types_stmt_r = $product_types_stmt->fetch())
      {
        ?>
            <tr id="row_<?=$product_types_stmt_r['ID']?>">
              <th scope="row"><?=$inc++?></th>
              <td><?=$product_types_stmt_r['product_name']?></td>
...
if(isset($_POST['product_type_name']) && isset($_POST['postid']))
{
  ...
  if(trim($product_type_name)!='')
  {
    if($postid == '0')
    {
      ...
      if($pdo->query("UPDATE `product_types` SET `product_name` = '".addslashes($product_type_name)."' WHERE  `ID` = '".$postid."';');") == true){
        echo "1";
      }else{
        echo "0";
      }
